...
if (!options.credentials && hasKey) options.keyFilename = keyPath;
...
async function readJsonOr<T>(filePath: string, fallback: T): Promise<T> {
  try {
    ...
    return c.trim() ? (JSON.parse(c) as T) : fallback;
  } catch {
    return fallback;
  }
}

async function migrateCollection(colName: string, items: any[], getDocId: (x: any) => string) {
...
async function main() {
  ...
  const avatars = await readJsonOr<any[]>(files.avatars, []);
  const states = await readJsonOr<any[]>(files.states, []);
  const memories = await readJsonOr<any[]>(files.memories, []);
  const persons = await readJsonOr<any[]>(files.persons, []);
  const relationships = await readJsonOr<any[]>(files.relationships, []);
