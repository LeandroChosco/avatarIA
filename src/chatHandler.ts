...
import type { AvatarProfile, AvatarState, Memory, Mood, Person } from "./types.js";
import {
  getOrCreateAvatar,
  addMemory,
  getTopKMemoriesByEmbedding,
  getRecentMemories,
  saveAvatarState,
  getPeople,
  getRelationships,
...
function detectEmotion(text: string): Mood {
  const t = text.toLowerCase();
  if (/feliz|content[oa]|alegr[ía]/.test(t)) return "feliz";
  if (/trist[ea]|melanc|llor|deprim/.test(t)) return "triste";
  if (/ansios|estres|nervios/.test(t)) return "ansiosa";
  if (/emocionad|entusiasm/.test(t)) return "entusiasmada";
  if (/nostal[gí]a|record/.test(t)) return "nostálgica";
  return "neutral";
}
...
export function extractNewMemories(userMsg: string, avatarReply: string, avatarId: string): Memory[] {
...
export async function handleChat(avatarId: string, userMessage: string): Promise<string> {
  ...
  const memorySet: Memory[] = lastDiary
  ...
  try {
    ...
    if (people.length) {
      ...
      const relByPerson: Record<string, number> = Object.create(null);
      for (const r of relationships) relByPerson[r.personId] = Math.max(relByPerson[r.personId] || 0, r.cercania || 0);
      const keyOrder = (name: string) => ({ sofia: 0, enrique: 1, fernando: 2 }[name.toLowerCase()] ?? 9);
      valid.sort((a, b) => {
      ...
      const roleLabel = (name: string, rol?: string) => (rol ? `${name} (${rol})` : name);
      ...
      const relNotes = relationships
        ...
        .join(" | ");
      if (relNotes) relationContext += `\nNotas recientes: ${relNotes}`;
      // Hechos relacionales explícitos (si están presentes)
      const byName = (name: string) => valid.find((p) => p.nombre.normalize("NFD").replace(/\p{Diacritic}/gu, "").toLowerCase() === name);
      const f = byName("fernando");
      const s = byName("sofia");
      const e = byName("enrique");
      const facts: string[] = [];
      if (f) facts.push("Fernando es su compañero de trabajo");
      if (s) facts.push("Sofía es su jefa");
      if (e) facts.push("Enrique es su hermano");
      if (facts.length) relationContext += `\nHechos relacionales: ${facts.join("; ")}`;
    }
    if (!relationContext) {
  ...
  if (apiKey) {
    ...
    const res = await client.chat.completions.create({
      ...
      messages: [
        ...
        ...(relationContext ? [{ role: "system", content: relationContext }] as const : []),
        ...(userStyle ? [{ role: "system", content: userStyle }] as const : []),
  ...
  for (const m of memories) await addMemory(m);
  ...
  if (Math.random() < 0.35) {
    ...
    if (peopleCache.length) {
      for (const p of peopleCache) {
        ...
        if (name && t.includes(name)) {
          const variants = [
            (n: string) => `y ${n} que dijo?`,
            (n: string) => `y con ${n} al final q paso?`,
            (n: string) => `q onda con ${n} entonces`,
          ];
    ...
    // Temas detectables
    if (/cine|pel[ií]cula|superman|funci[oó]n/.test(t)) followups.push("como estuvo la peli?");
    if (/trabajo|proyecto|colecci[oó]n|dise[nñ]o|ventas?/.test(t)) followups.push("y eso en el trabajo como resulto?");
    if (/familia|hermano|hermana|mam[aá]|pap[aá]/.test(t)) followups.push("y en casa todo bien?");
    if (/caf[eé]|cafe/.test(t)) followups.push("te peg[oó] el cafecito?");
    if (/clase|alumno|idioma/.test(t)) followups.push("te fue bien con la clase?");
...
export async function generateOpening(avatarId: string): Promise<string> {
  const { profile, state } = await getOrCreateAvatar(avatarId);
  await ensureDefaultRelations(avatarId);
  ...
  if (apiKey) {
    ...
    const res = await client.chat.completions.create({
      ...
      messages: [
        ...
        ...(style ? [{ role: "system", content: style }] as const : []),
  ...
  if (texts.some((t) => /pel[ií]cul|cine|superman/.test(t))) return pick(["y q tal estuvo la película?", "como te fue en el cine? jajaja"]);
  if (diary) return pick(["hoy anduve movida con eso q te conté", "te conté lo de hoy? estuvo raro jajaja"]);
