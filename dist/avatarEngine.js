...
function heuristicSeed(profile, state, lastSeeds) {
    const types = ["profesional", "social", "hobby", "emocional"];
    let tipo = rand(types);
    // Simple tendency based on energy and mood
    if (state.energia < 0.3)
        tipo = rand(["emocional", "hobby"]);
    if (state.mood === "entusiasmada")
        tipo = rand(["profesional", "social"]);
    let semilla = "";
    switch (tipo) {
        case "profesional":
...
function heuristicEvent(seed, profile, state) {
...
export async function generateDailySeed(profile, state, lastSeeds) {
    const openaiApiKey = getOpenAIKey();
    if (!openaiApiKey)
        return heuristicSeed(profile, state, lastSeeds);
...
export async function generateEventFromSeed(seed, profile, state, personName, personRole) {
    const openaiApiKey = getOpenAIKey();
    if (!openaiApiKey)
        return heuristicEvent(seed, profile, state);
    const client = new OpenAI({ apiKey: openaiApiKey });
...
export function applyEventToState(event, state) {
...
export async function tickAllAvatars() {
    ...
    for (const profile of profiles) {
        ...
        if (maybePerson) {
            const variants = [
                (n) => `anduve con ${n} hace un rato, salió algo random jajaja`,
                (n) => `me crucé con ${n} y termino en una mini odisea`,
                (n) => `estuve con ${n} y hubo un detalle q te va a encantar`,
                (n) => `lo de hoy con ${n} estuvo fuerte pero lindo`,
            ];
            const v = variants[Math.floor(Math.random() * variants.length)];
            spontaneous.push(v(maybePerson.nombre));
        }
        if (newState.mood === "entusiasmada")
            spontaneous.push("ando con mil ideas en la cabeza");
        if (spontaneous.length)
            await addNotification(profile.id, spontaneous);
        processed++;
    }
    return processed;
}
