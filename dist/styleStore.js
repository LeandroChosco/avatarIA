...
async function ensureFile() {
    try {
        ...
        await fs.writeFile(STYLES_FILE, "[]\n", "utf8");
    }
}
...
export async function analyzeAndUpdateUserStyle(avatarId, userMessage) {
    ...
    for (const ab of ABBREV) {
        ...
        if (hits)
            s.abbrevHits[ab] = (s.abbrevHits[ab] || 0) + hits;
    }
    ...
    if (alpha > 0 && lowerAlpha / alpha > 0.7)
        s.lowercaseDominant += 1;
    s.lastUpdated = new Date().toISOString();
    await writeAll(all);
}
export async function getUserStyleGuidance(avatarId) {
    const all = await readAll();
    const s = all.find((x) => x.avatarId === avatarId);
    if (!s || s.totalMessages < 3)
        return ""; // need some samples
    ...
    const prefs = [];
    if (emojiRate > 0.2)
        prefs.push("usa emojis a menudo");
    if (laughRate > 0.3)
        prefs.push("incluye 'jajaja' seguido de nada (sin punto)");
    if (openRate < 0.2)
        prefs.push("evita signos de apertura");
    if (dotRate < 0.3)
        prefs.push("no termines con punto final");
    if (lowerDomRate > 0.5)
        prefs.push("usa minúsculas la mayor parte del tiempo");
    if (exRate > 0.5)
        prefs.push("usa '!' con frecuencia");
    if (qRate > 0.5)
        prefs.push("haz preguntas a menudo");
    if (topAbbrev.length)
        prefs.push(`usa abreviaturas como: ${topAbbrev.join(", ")}`);
    if (!prefs.length)
        return "";
